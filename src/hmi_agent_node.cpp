...
void joystick_status_callback(const rio_control_node::Joystick_Status &joystick_status)
{
    ...
    output_signals.drivetrain_left_right = 0.5 * turn;
    ...
    output_signals.retract_intake = button_box_2_joystick->getButton(bb2_retract_intakes_button_id) || output_signals.intake_rollers || arm_joystick->getButton(arm_retract_intake_button_id);
    ...
    output_signals.manual_outake_back = button_box_2_joystick->getButton(bb2_manual_outtake_back_button_id) || (arm_joystick->getPOV(arm_pov_id) == arm_manual_outtake_back_pov_angle);
    output_signals.manual_outake_front = button_box_2_joystick->getButton(bb2_manual_outtake_front_button_id) || (arm_joystick->getPOV(arm_pov_id) == arm_manual_outtake_front_pov_angle);
